...
aws s3 cp "s3://${S3_BUCKET}/${S3_INPUT_FOLDER}" /tmp --recursive --exclude "*" --include "*.pgp"
...
# Upload the decrypted file to S3
  aws s3 cp "$output_file" "s3://${S3_BUCKET}/${S3_OUTPUT_FOLDER}${filename%.pgp}"
done

# Remove the temporary encrypted files
#rm /tmp/decrypted-files-uat/*.pgp
